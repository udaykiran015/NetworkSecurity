...
from networksecurity.entity.artifact_entity import DataIngestionArtifact,DataValidationArifact
...
class DataValidation:
    ...
    def validate_numerical_columns_exist(self,dataframe:pd.DataFrame)->bool:
        try:
            ...
            missing_columns = [column for column in numerical_columns if column not in dataframe.columns]
            if missing_columns:
    ...
    def detect_data_drift(self,base_df,current_df,threshold=0.05)->bool:
        try:
            ...
            for column in base_df.columns:
                a1=base_df[column]
                b1=current_df[column]
                is_same_dist=ks_2samp(a1,b1)
    ...
    def initiate_data_validation(self)->DataValidationArifact:
        try:
            train_file_path=self.data_ingestion_artifact.train_file_path
            test_file_path=self.data_ingestion_artifact.test_file_path
            ## Read The data from the ingestion path
            ...
            if not status:
                error_message += "Train Dataframe doesn't contain all the columns.\n"
            status=self.validate_number_of_columns(dataframe=test_dataframe)
            if not status:
                error_message += "Test Dataframe doesn't contain all the columns.\n"
            ...
            data_validation_artifact=DataValidationArifact(
